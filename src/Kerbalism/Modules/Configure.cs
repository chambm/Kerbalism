...
namespace KERBALISM
{
	...
	public class Configure : PartModule, IPartCostModifier, IPartMassModifier, IModuleInfo, ISpecifics, IConfigurable
	{
		...
		void Render_panel(Panel p, ConfigureSetup setup, int selected_i, int setup_i)
		{
			...
			string titleText = Lib.Ellipsis(setup.title, Styles.ScaleStringLength(70));
			...
			string sectionDesc = isExpanded ? string.Empty : setup.desc;
			if (canReconfigure)
			{
			...
			if (isExpanded)
			{
				...
				for (int i = 0; i < unlocked.Count; ++i)
				{
					...
					string optionTitle = Lib.Ellipsis(option.title, Styles.ScaleStringLength(60));
					...
					p.AddContent(optionTitle, marker, option.desc, () => Select_setup(capturedSlot, capturedIndex));
				}
			}
			else
			{
